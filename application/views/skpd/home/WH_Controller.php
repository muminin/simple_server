...
class WH_Controller extends CI_Controller
{
    ...
    public function set_nav()
    {
        ...
        if ($count_nav > 4) {
            ...
            // MENU WEB
            if (strpos($navv, 'text_jalan') !== false) {
                $active = NAVP_MENU_WEB;
                $nav = NAVC_MENU_WEB_1;
            } elseif (strpos($navv, 'profil') !== false) {
                $active = NAVP_MENU_WEB;
                $nav = NAVC_MENU_WEB_2;
            } elseif (strpos($navv, 'personil') !== false) {
                $active = NAVP_MENU_WEB;
                $nav = NAVC_MENU_WEB_2;
            } elseif (strpos($navv, 'agenda') !== false) {
                $active = NAVP_MENU_WEB;
                $nav = NAVC_MENU_WEB_3;
            } elseif (strpos($navv, 'tupoksi') !== false) {
                $active = NAVP_MENU_WEB;
                $nav = NAVC_MENU_WEB_4;
            }
            // SETTING
            elseif (strpos($navv, 'lemari') !== false) {
                $active = NAVP_PENGATURAN;
                $nav = NAVC_PENGATURAN_2;
            } // DOKUMENTASI HUKUM
            elseif (strpos($navv, 'prokum') !== false) {
                $active = NAVP_DOKUM;
                if (strpos($navv, 'daerah') !== false) {
                    $nav = NAVC_DOKUM_1;
                } elseif (strpos($navv, 'pusat') !== false) {
                    $nav = NAVC_DOKUM_2;
                } elseif (strpos($navv, 'non') !== false) {
                    $nav = NAVC_DOKUM_3;
                }
            }
            // REPORT PRODUK HUKUM
            elseif (strpos($navv, 'report') !== false) {
                $active = NAVP_REPORT;
                if (strpos($navv, 'penyusunan') !== false) {
                    $nav = NAVC_REPORT_2;
            ...
            // PENGAJUAN
            elseif (strpos($navv, 'pengajuan') !== false) {
                $active = NAVP_PROKUM;
                if (strpos($navv, 'pelacakan') !== false) {
                    $active = NAVP_REPORT;
                    $nav = NAVC_REPORT_2;
                } elseif (strpos($navv, 'daftar_isi') !== false) {
                    $active = NAVP_REPORT;
                    $nav = NAVC_REPORT_3;
                }  elseif (strpos($navv, 'penyusunan') !== false) {
                    $nav = NAVC_PROKUM_2;
                } else {
                    $nav = NAVC_PROKUM_1;
                }
            ...
            // KONSULTASI
            elseif (strpos($navv, 'konsultasi') !== false) {
                $active = "";
                $nav = NAVC_KONSULTASI;
            }
        } else {
            ...
            // DASHBOARD
            if (strpos($navv, 'dashboard') !== false) {
                $active = "";
                $nav = NAVC_DASHBOARD;
            }
            // SETTING
            elseif (strpos($navv, 'user') !== false) {
                $active = NAVP_PENGATURAN;
